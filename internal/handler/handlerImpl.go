...
func (s *SwiftCodeHandlers) Create(c *gin.Context) {
	...
	if s.isAnyFieldNil(swiftCode) {
		c.JSON(http.StatusBadRequest, model.ApiResponse{
			Message: "You need to provide Swift Code and Data. All fields must be filled",
		})
		return
	}
	err := s.srvInst.Create(swiftCode)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		s.HandlerLog.WithError(err).Error("Failed to create Swift Code")
		c.JSON(http.StatusInternalServerError, model.ApiResponse{
			Message: "The country does not exist. If you want, you need to insert the new country and ISO2 code first",
		})
		return
	}
